	...
	if( i< 6 || chr=='D' || chr=='A')
				switch (chr)
				{
					...
					case 'D' :            									//删除
								...
								{
									system("madplay -a +18 0005.mp3 &");	//通过system（）函数 来实现shell指令的调用
									i--;
					...
					case 'A' :												//确认
					            if(i==6)
								{
									...
									show_bmp("1.bmp",0,0);
									buf[i]='\0';							//字符串操作<string>的需要
									usleep(500);
									system("killall -9 madplay");           //dsp busy终止
									//比较验证码，--串口发送验证信息
									if(  !strcmp(buf,"016722") || !strcmp(buf,"119078") || !strcmp(buf,"900173")   )
									{
										buf[i]='A';                         //串口接受结束标识符
										buf[i+1]='\0';						//字符串操作<string>需要
										printf("%s\n",buf);
										printf("%d\n",strlen(buf));
										usart_send(buf);                    //串口发送验证信息
										usleep(400000);                     //连续串口发送延时
